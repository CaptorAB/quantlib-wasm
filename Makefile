...
build_bindings_from_unix:
	docker pull captorab/emscripten-quantlib:1.19.3
	docker run --mount type=bind,source="${PWD}",target=/src -it captorab/emscripten-quantlib:1.19.3 make


build_docker_image:
	docker build -t docker.io/captorab/emscripten-quantlib:1.19.3 .
#docker push docker.io/captorab/emscripten-quantlib:1.19.3
