...
build_bindings_from_unix:
	docker pull captorab/emscripten-quantlib:1.19.1
	docker run --mount type=bind,source="${PWD}",target=/src -it captorab/emscripten-quantlib:1.19.1 make


build_docker_image:
	docker build -t docker.io/captorab/emscripten-quantlib:1.19.1 .
#docker push docker.io/captorab/emscripten-quantlib:1.19.1
